        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <x-partials.card>
                ...
                <div class="mt-4 px-4">
                    ...
                    <div class="mb-4">
                        <h5 class="font-medium text-gray-700">
                            @lang('crud.payments.inputs.receipt_id')
                        </h5>
                        <span
                            >{{ optional($payment->receipt)->id ?? '-' }}</span
                        >
                    </div>
                    <div class="mb-4">
                        <h5 class="font-medium text-gray-700">
                            @lang('crud.payments.inputs.total_payment')
                        </h5>
                        <span>{{ $payment->total_payment ?? '-' }}</span>
                    </div>
                    <div class="mb-4">
                        <h5 class="font-medium text-gray-700">
                            @lang('crud.payments.inputs.total_price')
                        </h5>
                        <span>{{ $payment->total_price ?? '-' }}</span>
                    </div>
                    <div class="mb-4">
                        <h5 class="font-medium text-gray-700">
                            @lang('crud.payments.inputs.total_change')
                        </h5>
                        <span>{{ $payment->total_change ?? '-' }}</span>
                    </div>
                </div>
